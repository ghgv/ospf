...
def parseIpHdr(msg, verbose=1, level=0):
    ...
    if verbose > 1: print(prtbin(level*INDENT, msg[:IP_HDR_LEN]) ,(verhlen, tos, iplen, ipid, frag, ttl, proto, cksum, src, dst) = struct.unpack(IP_HDR, msg))

    ver  = (verhlen & 0xf0) >> 4
    hlen = (verhlen & 0x0f) * 4

    if verbose > 0:
        print ((level*INDENT +"IP (len=%d)" % len(msg))
        print ((level+1)*INDENT + "ver:%s, hlen:%s, tos:%s, len:%s, id:%s, frag:%s, ttl:%s, prot:%s, cksm:%" %  (ver, hlen, int2bin(tos), iplen, ipid, frag, ttl, proto, cksum) )
        print ((level+1)*INDENT + "src:%s, dst:%s" % (id2str(src), id2str(dst)))
